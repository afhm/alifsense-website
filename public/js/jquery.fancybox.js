(function (window, document, $, undefined) {
  ...
  $.extend(FancyBox.prototype, {
    init: function () {
      ...
      if (!$.fancybox.getInstance()) {
        ...
        if ($.fancybox.isTouch) {
          ...
          if (galleryHasHtml) {
            $('body').css({ position: 'fixed', width: testWidth, top: self.scrollTop * -1 });
          }
        } else {
          testWidth = $('body').width() - testWidth;
          if (testWidth > 1) {
            $('<style id="fancybox-noscroll" type="text/css">')
              .html('.compensate-for-scrollbar, .fancybox-enabled body { margin-right: ' + testWidth + 'px; }')
              .appendTo('head');
          }
        }
      }
    ...
    scaleToFit: function (duration) {
      ...
      $.fancybox.animate(
        $what,
        null,
        { top: end.top, left: end.left, scaleX: end.width / $what.width(), scaleY: end.height / $what.height() },
        duration || current.opts.speed,
        function () {
          self.isAnimating = false;
        }
    ...
    update: function (andSlides, andContent, duration, callback) {
      ...
      leftValue =
        self.current.pos * Math.floor(self.current.$slide.width()) * -1 - self.current.pos * self.current.opts.gutter;
      ...
      if (duration) {
        $.fancybox.animate(self.$refs.slider, null, { top: 0, left: leftValue }, duration, function () {
      ...
      if (opts.preload) {
        ...
        $slide.on('refresh.fb', function () {
          ...
          if ($body && $body.length && !(opts.css.width !== undefined && opts.css.height !== undefined)) {
            ...
            $wrap.css({
              width:
                opts.css.width === undefined ? frameWidth + ($wrap.outerWidth() - $wrap.innerWidth()) : opts.css.width,
              height:
    ...
    focus: function () {
      ...
      $el =
        current && current.isComplete
          ? current.$slide.find('button,:input,[tabindex],a:not(".disabled")').filter(':visible:first')
          : null;
      if (!$el || !$el.length) {
  ...
  $.fancybox = {
    ...
    isTouch:
      document.createTouch !== undefined && /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent),
    use3d: (function () {
    ...
    if (swiping === true) {
      if (Math.abs(self.distance) > 10) {
        if (self.instance.group.length < 2) {
          ...
          (self.instance.opts.touch.vertical === 'auto' && $(window).width() > 800)
        ) {
          self.isSwiping = 'x';
        } else {
  ...
  $.extend(SlideShow.prototype, {
    ...
    set: function () {
      ...
      if (
        ...
        (self.instance.current.opts.loop || self.instance.currIndex < self.instance.group.length - 1)
      ) {
